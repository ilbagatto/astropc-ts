...
function interpolate(ye: number, djd: number): number {
  ...
  return ((djd - j0) * (d1 - d0)) / (j1 - j0) + d0;
}

function predict(ye: number): number {
  // all other cases
  ...
  if (ye < 948) {
    ...
    let result = polynome(t, 102.0, 102.0, 25.3);
...
/**
 * Approximate Delta-T in seconds for a given Julian day.
 * Delta-T = ET - UT
 *
 * ```ts
 * const dt = deltaT(djd);
 * const djdEt = djd + dt / 86400.0;
 * ```
